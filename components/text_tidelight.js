/* ─────────────────────────────────────────
   ...
   SVG on load.
     ...
     <em data-tidelight>why behind the what</em>

   The marker colour comes from the --tidelight-ink token
   ...
   Load it like the other chrome components, before </body>:
     <script src="components/text_tidelight.js"></script>
───────────────────────────────────────── */
(function () {
  /* Styles, injected once. The --tidelight-ink fallback keeps the
     marker visible even where the token isn't defined. */
  if (!document.getElementById('tidelight-styles')) {
    var style = document.createElement('style');
    style.id = 'tidelight-styles';
    style.textContent =
      '.tidelight{position:relative;display:inline-block;white-space:nowrap;}' +
      '.tidelight__ink{position:absolute;left:-0.22em;top:-0.18em;' +
        'width:calc(100% + 0.44em);height:calc(100% + 0.30em);' +
        'pointer-events:none;z-index:0;}' +
      '.tidelight__base{fill:rgb(var(--tidelight-ink,172,160,232));transition:fill .25s ease;}' +
      '.tidelight__cap{fill:color-mix(in srgb,rgb(var(--tidelight-ink,172,160,232)) 80%,#000 20%);transition:fill .25s ease;}' +
      '.tidelight:hover .tidelight__base{fill:color-mix(in srgb,rgb(var(--tidelight-ink,172,160,232)) 90%,#000 10%);}' +
      '.tidelight:hover .tidelight__cap{fill:color-mix(in srgb,rgb(var(--tidelight-ink,172,160,232)) 68%,#000 32%);}' +
      '.tidelight__text{position:relative;z-index:1;color:var(--color-accent-on-accent);}' +
      '@media (prefers-reduced-motion: reduce){.tidelight__base,.tidelight__cap{transition:none;}}';
  ...
  var SVG =
    '<svg class="tidelight__ink" aria-hidden="true" preserveAspectRatio="none" viewBox="0 0 137 25">' +
      ...
      '</mask>' +
      '<path class="tidelight__cap" mask="url(#{ID})" d="M0.823 22.9875C2.12222 24.6601 14.0523 22.9134 20.6665 22.8763L128.906 22.7028C130.406 22.7028 131.393 21.5171 131.665 20.5217C132.23 17.1425 132.726 15.8892 133.13 13.4661C133.535 11.043 133.968 8.40162 134.203 5.66436C134.217 5.49971 134.26 5.06964 134.295 4.58343C134.381 3.39728 133.639 2.52232 132.734 2.4758C116.977 1.66595 41.6503 1.76509 35.412 1.80003L15.5684 1.91116C11.8498 1.93197 5.72888 1.36927 3.82068 2.63216C2.72881 3.35479 2.59275 6.24212 2.32068 7.23742C1.75573 9.30088 1.72532 10.0775 1.32068 12.5006C0.916036 14.9237 1.31844 14.4746 0.820683 17.1061C0.438628 19.1258 -0.67937 21.0533 0.823 22.9875Z"/>' +
    '</svg>';

  var seq = 0;

  function mark(el) {
    if (el.getAttribute('data-tidelight-ready')) return;

    var text = el.textContent;
    var maskId = 'tidelight-cap-' + (++seq);

    el.classList.add('tidelight');
    el.innerHTML = SVG.replace(/\{ID\}/g, maskId) + '<span class="tidelight__text"></span>';
    /* Set text via textContent so any characters in the source are escaped. */
    el.querySelector('.tidelight__text').textContent = text;
    el.setAttribute('data-tidelight-ready', '1');
  }

  function mount() {
    var els = document.querySelectorAll('[data-tidelight]');
